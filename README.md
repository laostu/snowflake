...
### 自定义纪元
默认情况下，此包使用 Twitter 纪元：1288834974657（即 2010年11月4日 01:42:54）。你可以通过将 `snowflake.Epoch` 设置为以毫秒为单位的时间值来自定义纪元。
...
### 工作原理
每次生成 ID 时，工作流程如下：
* 使用 ID 的 41 位存储毫秒精度的时间戳。
* 然后在后续位中添加 NodeID。
* 最后添加序列号，从 0 开始，同一毫秒内每生成一个 ID 递增一次。如果在同一毫秒内生成的 ID 数量导致序列号溢出，生成函数将暂停直到下一毫秒。
